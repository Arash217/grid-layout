import { style } from '@vanilla-extract/css'

export const gridLayout = style({
  position: 'relative',
  ':before': {
    content: '',
    /* 
      The CSS variables --col-width and --row-height are set in the styles of GridLayout,
      alternatively we could have used vanilla extract vars to change the values
      of --col-width and --row-height but that requires installing a runtime which we want to avoid.
    */
    backgroundSize: `var(--col-width) var(--row-height)`,
    backgroundImage:
      'url(data:image/svg+xml;base64,)',
    height: '100%',
    width: '100%',
    position: 'absolute',
    left: 0,
    top: 0,
    boxShadow: '0px 0px 2px 1px #d1d1d1',
  },
})
